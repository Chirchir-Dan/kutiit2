...
export default function LandingPage() {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-4 md:px-6 text-center max-w-5xl mx-auto space-y-8 md:space-y-12 py-10 md:py-20 overflow-x-hidden">
      ...
      <div className="space-y-4 md:space-y-6 w-full">
        ...
        <p className="text-sm sm:text-base md:text-2xl text-slate-500 max-w-2xl mx-auto font-medium leading-relaxed px-2">
          ...
          <span className="text-slate-900 block md:inline mt-1 md:mt-0"> Built for the future, rooted in tradition.</span>
        </p>
      </div>

      {/* PRIMARY CTA - Full width on tiny screens */}
      <Button asChild size="lg" className="bg-slate-900 hover:bg-black text-white px-6 md:px-10 h-14 md:h-16 rounded-2xl font-bold uppercase text-[10px] md:text-xs tracking-widest shadow-2xl transition-all hover:scale-105 active:scale-95 w-full sm:w-auto">
      ...
      {/* PROMO CARD - Flex-col on mobile, flex-row on desktop */}
      <div className="w-full mt-4 md:mt-8 bg-emerald-600 rounded-[1.5rem] md:rounded-[3rem] p-6 md:p-12 text-white text-left flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8 shadow-2xl shadow-emerald-200 relative overflow-hidden">
      ...
      {/* SUPPORT SECTION - Commented out for now */}
       {mounted && (
        <div className="pt-4">
          <SupportModal>
