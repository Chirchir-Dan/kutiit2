  ...
  try {
    ...
    if (retrievedWords.length === 0) {
      return NextResponse.json({
        reply:
          "I couldn't find any relevant words in my Nandi database for your query. Please try different keywords or add these words to the dictionary first.",
        retrieved: [],
...
Respond naturally in Nandi using only available words. Follow the response structure in your instructions.`;
    ...
    const validation =
      aiReply === "No response generated."
        ? { isValid: true, unknownWords: [] }
        : validateNandiOutput(aiReply, retrievedWords);
